...
void CataCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P, float image_scale) const {
...
void CataCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p, float image_scalse) const {
 ...
 * \param p return value, contains the image point coordinates
 */
void CataCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p, Eigen::Matrix<double, 2, 3>& J) const {
  double xi = mParameters.xi();
...
cv::Size CataCamera::imageSize() const { return cv::Size(imageWidth(), imageHeight()); }
}  // namespace camera_model
