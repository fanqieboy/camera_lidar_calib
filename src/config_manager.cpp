...
bool ConfigManager::LoadConfig(const std::string& config_path) {
  try {
    ...
    *config_node_ = YAML::LoadFile(config_path);
    ...
    // 第二步：如果指定了data_path，尝试自动搜索文件
    if (!params_.data_path.empty()) {
        if (SearchDataFiles()) {
            std::cout << "[ConfigManager] 自动搜索完成:" << std::endl;
            std::cout << "  - 图像: " << params_.image_path << std::endl;
            std::cout << "  - 点云: " << params_.pcd_path << std::endl;
        }
    }

    // 第三步：从关联的相机模型文件中加载内参和畸变系数
    if (!params_.camera_model_path.empty()) {
        if (LoadCameraModel(params_.camera_model_path)) {
            std::cout << "[ConfigManager] 成功从模型文件加载参数: " << params_.camera_model_path << std::endl;
        } else {
            // 注意：这里不强制返回失败，如果没有模型文件，允许回退到主配置加载的内容
            std::cout << "[ConfigManager] 警告: 无法加载关联的模型文件 " << params_.camera_model_path << std::endl;
        }
    }
    ...
    std::cout << "[ConfigManager] 成功加载配置系统" << std::endl;
    PrintParams();
    return true;
  } catch (const std::exception& e) {
    last_error_ = "加载配置文件失败: " + std::string(e.what());
    return false;
  }
}

bool ConfigManager::LoadCameraModel(const std::string& model_file) {
    try {
        // PCL/OpenCV 风格的 YAML 可能包含 %YAML:1.0，yaml-cpp 有时需要处理这种头部
        // 或者直接尝试 LoadFile
        YAML::Node config = YAML::LoadFile(model_file);
        
        if (config["projection_parameters"]) {
            auto params = config["projection_parameters"];
            
            // 映射关系：mu->fx, mv->fy, u0->cx, v0->cy
            if (params["mu"]) params_.fx = params["mu"].as<double>();
            if (params["mv"]) params_.fy = params["mv"].as<double>();
            if (params["u0"]) params_.cx = params["u0"].as<double>();
            if (params["v0"]) params_.cy = params["v0"].as<double>();
            
            // 映射畸变系数: k2, k3, k4, k5 -> distortion_coeffs
            params_.distortion_coeffs.clear();
            if (params["k2"]) params_.distortion_coeffs.push_back(params["k2"].as<double>());
            if (params["k3"]) params_.distortion_coeffs.push_back(params["k3"].as<double>());
            if (params["k4"]) params_.distortion_coeffs.push_back(params["k4"].as<double>());
            if (params["k5"]) params_.distortion_coeffs.push_back(params["k5"].as<double>());
            
            return true;
        }
    } catch (const std::exception& e) {
        last_error_ = "相机模型加载错误 (" + model_file + "): " + std::string(e.what());
        return false;
    }
    return false;
}

bool ConfigManager::SearchDataFiles() {
    if (params_.data_path.empty()) return false;
    
    std::string data_path = params_.data_path;
    if (!std::filesystem::exists(data_path) || !std::filesystem::is_directory(data_path)) {
        last_error_ = "data_path 不存在或不是目录: " + data_path;
        return false;
    }
    ...
    // 清空旧路径，确保完全由搜索决定
    params_.image_path.clear();
    params_.pcd_path.clear();
    ...
    for (const auto& entry : std::filesystem::directory_iterator(data_path)) {
        ...
        if (!found_image) {
            if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp") {
                params_.image_path = entry.path().string();
                found_image = true;
            }
        }
        ...
        if (!found_pcd) {
            if (ext == ".pcd") {
                params_.pcd_path = entry.path().string();
                found_pcd = true;
            }
        }
        
        if (found_image && found_pcd) break;
    }
    
    return found_image && found_pcd;
}
...
void ConfigManager::PrintParams() const {
  ...
  std::cout << "  pcd_path: " << params_.pcd_path << std::endl;
...
void ConfigManager::SetDefaultParams() {
  ...
  // 保留兼容性，但独立版本不使用
  params_.pcd_path = "./data/input.pcd";
...
bool ConfigManager::ParseYamlNode(const YAML::Node& config) {
  try {
    ...
    // 解析畸变系数 (支持鱼眼 4 参数 和 针孔 5+ 参数)
    if (config["distortion_coeffs"]) {
      params_.distortion_coeffs.clear();
      for (const auto& coeff : config["distortion_coeffs"]) {
        params_.distortion_coeffs.push_back(coeff.as<double>());
      }
    }
    ...
    // 解析文件路径
    if (config["data_path"]) params_.data_path = config["data_path"].as<std::string>();
