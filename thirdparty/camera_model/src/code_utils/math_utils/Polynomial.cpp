...
eigen_utils::Vector Polynomial::getRealRoot(double _y) {
  ...
  for (int i = 0; i < realRoots.size(); i++) {
    if (imagRoots(i) == 0) realRoot = eigen_utils::pushback(realRoot, realRoots(i));
  }
  // std::cout<<"real roots: "<<realRoot<<std::endl;
  return realRoot;
}

eigen_utils::Vector Polynomial::getRealRoot(double _y, double x_min, double x_max) {
  eigen_utils::Vector realRoots, imagRoots;
...
void Polynomial::setPolyCoeff(const eigen_utils::Vector& value) { m_coeff = value; }

void Polynomial::setPolyCoeff(int order_index, double value) { m_coeff(order_index) = value; }

eigen_utils::Vector Polynomial::getPolyCoeff() const { return m_coeff; }

double Polynomial::getPolyCoeff(int order_index) const { return m_coeff(order_index); }
...
bool Polynomial::FindRoots(const double y, const eigen_utils::Vector& polynomial_in, eigen_utils::Vector& real,
...
void Polynomial::FindLinearPolynomialRoots(const eigen_utils::Vector& polynomial, eigen_utils::Vector& real,
...
void Polynomial::FindQuadraticPolynomialRoots(const eigen_utils::Vector& polynomial, eigen_utils::Vector& real,
                                              eigen_utils::Vector& imag) {
...
void Polynomial::BuildCompanionMatrix(const eigen_utils::Vector& polynomial,
...
void Polynomial::BalanceCompanionMatrix(eigen_utils::Matrix* companion_matrix_ptr) {
  eigen_utils::Matrix& companion_matrix = *companion_matrix_ptr;
...
PolynomialFit::PolynomialFit(int _order, eigen_utils::Vector _x, eigen_utils::Vector _y) : Polynomial(_order) {
  poly = new Polynomial(_order);
  samples.clear();
  data_size = 0;
...
void PolynomialFit::loadSamples(const eigen_utils::Vector& x, const eigen_utils::Vector& y) {
