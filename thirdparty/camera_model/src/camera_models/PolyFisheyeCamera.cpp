...
PolyFisheyeCamera::PolyFisheyeCamera(const std::string& cameraName, int imageWidth, int imageHeight, double k2,
                                     double k3, double k4, double k5, double k6, double k7, double p1, double p2,
                                     double A11, double A12, double A22, double u0, double v0, int isFast)
  ...
  if (mParameters.isFast() == 1) {
    eigen_utils::Vector coeff(FISHEYE_POLY_ORDER + 1);
    coeff << 0.0, 1.0, mParameters.k2(), mParameters.k3(), mParameters.k4(), mParameters.k5(), mParameters.k6(),
        mParameters.k7();
...
PolyFisheyeCamera::PolyFisheyeCamera(const PolyFisheyeCamera::Parameters& params) : mParameters(params) {
  eigen_utils::Vector coeff(FISHEYE_POLY_ORDER + 1);
...
Camera::ModelType PolyFisheyeCamera::modelType() const { return mParameters.modelType(); }

const std::string& PolyFisheyeCamera::cameraName() const { return mParameters.cameraName(); }
...
void PolyFisheyeCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p) const {
  ...
  if (mParameters.isDistortion()) {
    if (mParameters.isFast() == 1) {
      ...
      double r_point = r(mParameters.k2(), mParameters.k3(), mParameters.k4(), mParameters.k5(), mParameters.k6(),
                         mParameters.k7(), theta);
      //            Eigen::Vector2d du
 ...
 */
void PolyFisheyeCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p,
  ...
  if (mParameters.isFast() == 1)
    ...
    p_u = r(mParameters.k2(), mParameters.k3(), mParameters.k4(), mParameters.k5(), mParameters.k6(), mParameters.k7(),
            theta) *
          Eigen::Vector2d(cos_phi, sin_phi);
...
void PolyFisheyeCamera::liftSphere(const Eigen::Vector2d& p, Eigen::Vector3d& P) const { liftProjective(p, P); }

void PolyFisheyeCamera::rayToPlane(const Ray& ray, Eigen::Vector2d& p) const {
  Eigen::Vector2d p_u = r(mParameters.k2(), mParameters.k3(), mParameters.k4(), mParameters.k5(), mParameters.k6(),
                          mParameters.k7(), ray.theta()) *
...
void PolyFisheyeCamera::liftProjectiveToRay(const Eigen::Vector2d& p, Ray& ray) const {
  ...
  if (mParameters.isFast() == 1) {
    fastCalc->backprojectSymmetric(
        Eigen::Vector2d(m_inv_K11 * p(0) + m_inv_K12 * p(1) + m_inv_K13, m_inv_K22 * p(1) + m_inv_K23), cos_theta,
...
cv::Mat PolyFisheyeCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize,
...
const PolyFisheyeCamera::Parameters& PolyFisheyeCamera::getParameters() const { return mParameters; }
...
void PolyFisheyeCamera::backprojectSymmetric(const Eigen::Vector2d& p_u, double& cos_theta, double& sin_theta,
...
PolyFisheyeCamera::FastCalcTABLE* PolyFisheyeCamera::getFastCalc() { return fastCalc; }

void PolyFisheyeCamera::setFastCalc() {
  eigen_utils::Vector coeff_fast(FISHEYE_POLY_ORDER + 1);
  coeff_fast << 0.0, 1.0, mParameters.k2(), mParameters.k3(), mParameters.k4(), mParameters.k5(), mParameters.k6(),
      mParameters.k7();
...
PolyFisheyeCamera::Parameters::Parameters(const std::string& cameraName, int w, int h, double k2, double k3, double k4,
                                          double k5, double k6, double k7, double p1, double p2, double A11, double A12,
...
bool PolyFisheyeCamera::Parameters::isDistortion() const { return (m_k2 != 0.0); }
...
PolyFisheyeCamera::Parameters& PolyFisheyeCamera::Parameters::operator=(const PolyFisheyeCamera::Parameters& other) {
  if (this != &other) {
...
std::ostream& operator<<(std::ostream& out, const PolyFisheyeCamera::Parameters& params) {
...
eigen_utils::Matrix PolyFisheyeCamera::FastCalcTABLE::getMatAngleToR() { return angleToR; }

eigen_utils::Matrix PolyFisheyeCamera::FastCalcTABLE::getMatRToAngle() { return rToAngle; }

double PolyFisheyeCamera::FastCalcTABLE::getMaxIncidentAngle() { return maxIncidentAngle; }
...
bool PolyFisheyeCamera::FastCalcTABLE::calcAngleToR(eigen_utils::Matrix& _angleToR, const int _numDiffAngle,
...
bool PolyFisheyeCamera::FastCalcTABLE::calcRToAngle(eigen_utils::Matrix& _rToAngle, const int _numDiffR,
...
void PolyFisheyeCamera::FastCalcTABLE::backprojectSymmetric(const Eigen::Vector2d& p_u, double& cos_theta,
                                                            double& sin_theta, double& cos_phi, double& sin_phi) const {
  double r = p_u.norm();
  double theta;
...
void PolyFisheyeCamera::FastCalcPOLY::backprojectSymmetric(const Eigen::Vector2d& p_u, double& cos_theta,
                                                           double& sin_theta, double& cos_phi, double& sin_phi) const {
  double r = p_u.norm();
  //  std::cout << "#INFO: r is " << r << std::endl;
...
void PolyFisheyeCamera::FastCalcPOLY::resetFastCalc() {
  ...
  for (int index = 0; index <= numDiffAngle; ++index) thetas(index) = diffAngle * index;
